...
function adminer_object(): Adminer
{
    return new class extends Adminer
    {
        ...
        {
            switch ($key) {
                case 'db': return $this->getEnv('ADMINER_DEFAULT_DB_NAME');
                case 'driver': return $this->getEnv('ADMINER_DEFAULT_DB_DRIVER');
                case 'password': return $this->getEnv('ADMINER_PASSWORD');
                case 'server': return $this->getEnv('ADMINER_DEFAULT_SERVER');
